        ...
        tools: [
            {
                ...
                inputSchema: zodToJsonSchema(users.GetUserProfileSchema),
            },
            {
                name: "update_user_profile",
                description: "Update the authenticated user's profile information",
                inputSchema: zodToJsonSchema(users.UpdateUserProfileSchema),
            },
            {
                name: "list_own_repositories",
                description: "List repositories belonging to the authenticated user",
                inputSchema: zodToJsonSchema(users.ListOwnRepositoriesSchema),
            },
            {
    ...
    try {
        ...
        switch (request.params.name) {
            ...
            case "update_user_profile": {
                const args = users.UpdateUserProfileSchema.parse(request.params.arguments);
                const profile = await users.updateUserProfile(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(profile, null, 2) }],
                };
            }
            case "list_own_repositories": {
                const args = users.ListOwnRepositoriesSchema.parse(request.params.arguments);
                const repos = await users.listOwnRepositories(args);
                return {
