...
export function redact(value: unknown, seen = new WeakSet<object>()): unknown {
  if (typeof value === "string") return SECRET_VALUE.test(value) ? "[REDACTED]" : redactUrl(value);
...
function redactUrl(value: string): string {
  try { const url = new URL(value); if (url.search || url.hash) return `${url.origin}${url.pathname}[REDACTED]`; return value; } catch { return value; }
}
