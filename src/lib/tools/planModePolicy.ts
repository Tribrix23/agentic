...
export function buildPlanModeContract(): string {
  return [
    ...
    'You MUST call listDirectory with path "." first, then call readFile for the relevant source files before planning.',
    'Do not claim to have inspected files unless a tool result confirms it.',
    'After inspection, you MUST call writeFile with path "implementation_plan.md" and the complete Markdown plan. Use editFile with an exact anchor for later revisions.',
