...
const TechText = ({
  ...
  useEffect(() => {
    ...
    const refreshFonts = () => {
      ...
      let f = s.fontFamily || getComputedStyle(container).fontFamily || 'sans-serif';
      if (f === 'inherit') f = 'sans-serif';
      return f;
    };
    ...
    const setFont = (target: CanvasRenderingContext2D, s: Settings, size: number) => {
      ...
      if ('letterSpacing' in target as any) (target as any).letterSpacing = `${s.letterSpacing * size}px`;
    ...
    const ensureLayout = (s: Settings): Word => {
      ...
      if (document.fonts && wanted !== requestedFont) {
        ...
        document.fonts.load(wanted, s.text).then(refreshFonts, refreshFonts);
            ...
            const getMetrics = (ctx: CanvasRenderingContext2D, text: string, size: number) => {
        const m = ctx.measureText(text);
        return {
          left: m.actualBoundingBoxLeft || 0,
          right: m.actualBoundingBoxRight || m.width || size,
      ...
      let m1 = getMetrics(probe, s.text, s.fontSize);
      
      const fit = Math.min(
        1,
        (width * 0.9) / Math.max(m1.left + m1.right, 1),
        (height * 0.66) / Math.max(m1.ascent + m1.descent, 1)
      );
      const size = s.fontSize * fit;
      setFont(probe, s, size);
      
      let m = getMetrics(probe, s.text, size);
      const inkWidth = m.left + m.right;
      const inkHeight = m.ascent + m.descent;
      const x = (width - inkWidth) / 2 + m.left;
      const baseline = (height - inkHeight) / 2 + m.ascent;
      const next = {
        size,
        baseline,
        left: x - m.left,
      ...
      chars.forEach((char, i) => {
        prefix += char;
                const own = getMetrics(probe, char, size);
        ...
        const base = {
          ...
          box: {
            x1: gx - own.left,
            y1: baseline - own.ascent,
            x2: gx + own.right,
            y2: baseline + own.descent
          }
        };
...
export default React.memo(TechText);
