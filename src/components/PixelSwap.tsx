...
const buildKeyframes = ({
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    if (reverse) eased = 1 - eased; // Play backwards
...
function PixelSwap({
  ...
  useEffect(() => {
    ...
    // Always use the second layer (index 1) for the curtain effect
    const source = layerRefs.current[1];
    ...
    const keyframes = buildKeyframes({
      ...
      fade: settings.fade,
      reverse: !to // Reverse when going out
    });
  ...
  const renderLayer = (content: ReactNode, index: number) => {
    ...
    return (
      <div
        ...
        data-visible={isShown && !(transition && index === 1)}
